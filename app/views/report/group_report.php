              ...
              <div style="display: block; width: 100%; height: 70vh; margin-left: 10px; margin-right: 50px; overflow-y: scroll; background: #fff; border: solid #eee;">
                <div class="table-responsive">
                  <table class="table table-bordered">
                    <thead>
                      <tr>
                        <th scope="col" class="col-2">Member</th>
                    ...
                    <? foreach($reports as $uid => $report): ?>
                      ...
                      <? foreach($categories as $category): ?>
                        <td><?= $report['content'][$category['category_id']]['content'] ?? '' ?></td>
                      <? endforeach ?>
                        <td align="center">
                        <? if($uid == Auth::uid()): ?>  
                          <a href="<?= Helper::fullPath('/reports/' . $group['group_id'] . '/' . $month . '/update') ?>" class="btn btn-primary"><i class="fas fa-edit"></i></a>
                        <? endif ?>
                        </td>                           
                      </tr>
                    <? endforeach ?>
